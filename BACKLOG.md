...
### Shipped in v3.2 (28 Aug 2026)

- **#53 -- Backup/restore scope selection** (headline) -- export takes a scope (Database only / Settings only / Database + Settings), returned as a zip carrying `manifest.json` plus `history.db` and/or `config.ini`. Resolves the long-standing scope decision: rather than pick one policy for everyone, the user chooses per action. Import auto-detects the new zip format and still accepts legacy raw gzip/`.db` backups unchanged. `config.ini` restores stage to `config.ini.pending` alongside `history.db.pending` and apply on next start via `_apply_pending_imports()` (renamed from singular), backing up whatever they replace. A combined restore failing validation on either file is rejected whole, never half-applied. **The automatic daily backup deliberately stays database-only** -- it's a silent safety net, not a user-facing migration action.
- **Year in Review annual cost projection made realistic** -- the projection scaled recorded £ totals to a full year by the kWh ratio, which assumed the rest of the year would be as cheap to run as the period on record. On a young install holding only summer data that projected a net annual *earning* for a household that really pays a net cost (caught against Andi's own 11 months of Octopus data). Remaining months are now estimated by netting the seasonal curves' monthly solar against monthly consumption (`_remaining_import_export_kwh`): only the shortfall becomes import, only the surplus becomes export. Added `_avg_tou_export_rate()` mirroring the existing import helper. Hand-verified on real data: same input went from -£59.81 (net earning) to +£405.86 (net cost).
- **Out of Public Beta** -- the "Public Beta" label dropped from the site footer, hero badge, licence and user guide, and from the GitHub release title going forward. Historical release-notes entries keep their original label.
- **#53 "missing days" root-caused and closed out** -- not the theorised silent import failure. A tester's container was built by hand in Synology DSM Container Manager, which never applied the compose file's `TZ`, leaving it on `Etc/UTC`. Since history is grouped by calendar day via SQLite's `localtime`, a whole day could look empty. `install-docker.html` troubleshooting now covers non-compose setups (DSM, Portainer, Unraid).
- **Docs accuracy fixes** -- user guide's Backup & Restore section rewritten for the new scope selector, and a long-standing false claim corrected (it said the dashboard "restarts automatically after a successful restore"; a restore has always been staged and applied on the next manual restart). Stale `website/downloads/` copies of `update.sh` and `config.ini.example` refreshed -- the former was missing the VERSION copy and weather-icons step, the latter missing every config section added since June.
- Tests added: `tests/test_year_projection.py` (5). Full suite now 101.
...
| [#53](https://github.com/ACBCSoftware/acbc-givenergy-dashboard/issues/53) | Docker backup/restore scope + "missing days" | **Both sub-threads resolved.** Docker shipped v3.0; scope selection (Database / Settings / Both) shipped v3.2. "Missing days" root-caused to a container `TZ` left at `Etc/UTC` when built by hand in DSM Container Manager, not an import bug -- docs updated. Tester later reported a recurrence after rebuilding via compose (22 Jul), then moved to a Linux VM; **watch for any further report before closing** |
...
| [#57](https://github.com/ACBCSoftware/acbc-givenergy-dashboard/issues/57) | Met Office API key "too long" | **Fixed in v3.1, confirmed on hardware by the reporter (23 Jul)** -- safe to close |
